...
import br.com.matheus.projetolocadora.modelo.IOrder;
import br.com.matheus.projetolocadora.util.HibernateUtil;
...
public abstract class GenericoDAO<T, PK extends Serializable> {
    ...
    public Integer criar(T o) throws Exception{
    	Integer save;
    	Transaction tx = null;
        try{
            this.carregarSessao();
            tx = this.sessao.beginTransaction();
            save = (Integer)this.sessao.save(o);
            tx.commit();
        }catch(Exception ex){
            if(tx != null){
                tx.rollback();
